...
function buildRoadNetworkQuery(bounds) {
    return `
        [out:json][timeout:25];
        (
            way["highway"~"${PLAYABLE_HIGHWAY_REGEX}"]
...
const GameController = {
    ...
    enterPhase(newPhase, options = {}) {
        ...
        this._exitPhase(oldPhase, newPhase);
    ...
    _exitPhase(phase, nextPhase = null) {
        switch (phase) {
            ...
            case GamePhase.VISUALIZING:
                // The geographic history is already blended in before this handoff.
            ...
            case GamePhase.RESULTS:
                GameState.resultsPresentation.enteredAt = 0;
                break;
        }
    },
...
const CityFacts = {
    ...
    tickerVersion:0, resultVersion:0, transitionVersion:0,
    ...
    async fetchFacts(cityInput) {
        // Bundled city notes keep the offline testcase network-free.
        // Source: https://wwwx.miamidade.gov/global/management/municipalities.page
        if(cityInput?.packId==='miami')return ['Miami became an incorporated city on July 28, 1896.'];
    ...
    async showFactInResults(cityInput) {
        ...
        factEl.classList.add('hidden');factTextEl.textContent='';
        const fact = await this.getRandomFact(cityInput);
        if(version!==this.resultVersion)return;
    ...
    async showFactInTransition(cityInput) {
        ...
        if (!factEl) return;
        const version=++this.transitionVersion;factEl.textContent='';
        const fact = await this.getRandomFact(cityInput);
    ...
    async startTicker(cityInput) {
        ...
        const city = this.resolveCityInput(cityInput);
        const version=this.tickerVersion;
     ...
     */
    stopTicker() {
        this.tickerVersion++;this.resultVersion++;this.transitionVersion++;
        this.currentFact=null;
        for(const id of ['ticker-text','transition-fact','fact-text']){const el=document.getElementById(id);if(el)el.textContent='';}
        document.getElementById('location-fact')?.classList.add('hidden');
        const tickerEl = document.getElementById('facts-ticker');
    ...
    async updateTickerCity(cityInput) {
        if (!this.ticker.active&&!this.ticker.cityName) return;
        const city = this.resolveCityInput(cityInput);
        const cityName = city?.name || '';
        if (cityName === this.ticker.cityName) return;

        // Hide old content immediately, and give the replacement its own generation.
        await this.startTicker(city);
    },
};
...
const WebGLRenderer = {
    ...
    getCurrentColorIndex() {
        if (GameState.gameMode === 'visualizer') {
            return GameState.visualizerState.themeIndex ?? VisualizerHistory.pathIndex;
    ...
    renderAmbientGlow(time, width, height) {
        const visibility = getAmbientRoadVisibility();
...
const SoundEngine = {
    ...
    // Hovering route preview lock chirp, derived from existing found sample.
    previewLock(signature) {
        if (!this.initialized || this.muted || !signature) return;

        const nowMs = performance.now();
        if (this.previewCueState.signature === signature && nowMs - this.previewCueState.lastAt < 320) {
            return;
        }
        if (nowMs - this.previewCueState.lastAt < 90) {
            return;
        }

        this.previewCueState.signature = signature;
        this.previewCueState.lastAt = nowMs;

        if (this.buffers.found) {
            const now = this.ctx.currentTime;
            const source = this.ctx.createBufferSource();
            source.buffer = this.buffers.found;
            source.playbackRate.value = 2.15 + Math.random() * 0.2;
            ...
            gain.gain.setTargetAtTime(0.001, now + 0.05, 0.025);
            ...
            const offset = Math.min(0.045, Math.max(0, this.buffers.found.duration - 0.08));
            const duration = Math.min(0.075, Math.max(0.03, this.buffers.found.duration - offset));
            source.start(now, offset, duration);
            source.stop(now + duration / source.playbackRate.value + 0.03);
            return;
        }

        // Fallback if sample hasn't loaded yet.
        this.hover();
    },
...
const AmbientViz = {
    ...
    renderRoundReviewBackdrop(ctx, highlightedPaths, overlayAlpha = 0.52) {
        ...
        ctx.save();
        ctx.globalCompositeOperation = 'source-over';
        ctx.fillStyle = `rgba(6, 8, 16, ${overlayAlpha})`;
        ctx.fillRect(0, 0, width, height);

        ctx.globalCompositeOperation = 'destination-out';

        for (const points of highlightedPaths) {
            ...
            const focusAnchors = [points[0], points[Math.floor(points.length * 0.5)], points[points.length - 1]];
            for (const anchor of focusAnchors) {
                if (!anchor) continue;
                const gradient = ctx.createRadialGradient(anchor.x, anchor.y, 0, anchor.x, anchor.y, 82);
                gradient.addColorStop(0, 'rgba(0, 0, 0, 0.42)');
                gradient.addColorStop(0.55, 'rgba(0, 0, 0, 0.18)');
                gradient.addColorStop(1, 'rgba(0, 0, 0, 0)');
                ctx.fillStyle = gradient;
                ctx.beginPath();
                ctx.arc(anchor.x, anchor.y, 82, 0, Math.PI * 2);
                ctx.fill();
            }
        }
    ...
    renderResultsUserPath(ctx, points, intensity = 1) {
        if (points.length < 2) return;

        const time = performance.now() * 0.001;
        const white = CONFIG.color.getUserPathColor();
        const accent = { r: 255, g: 190, b: 122 };
        const shimmer = 0.72 + 0.28 * Math.sin(time * 4.2);
        const effectiveIntensity = Math.max(0, Math.min(1.25, intensity * shimmer));
        ...
        ctx.globalCompositeOperation = 'source-over';
        ctx.strokeStyle = `rgba(0, 0, 0, ${0.52 + 0.2 * intensity})`;
        ctx.lineWidth = 18;
        buildClippedPath();
        ctx.stroke();

        ctx.strokeStyle = `rgba(18, 20, 32, ${0.75 + 0.1 * intensity})`;
        ctx.lineWidth = 12;
        buildClippedPath();
        ctx.stroke();
        ...
        ctx.strokeStyle = `rgba(${white.r}, ${white.g}, ${white.b}, ${0.18 + 0.08 * effectiveIntensity})`;
        ctx.lineWidth = 30;
        buildClippedPath();
        ctx.stroke();

        ctx.strokeStyle = `rgba(${accent.r}, ${accent.g}, ${accent.b}, ${0.08 + 0.08 * effectiveIntensity})`;
        ctx.lineWidth = 18;
        buildClippedPath();
        ctx.stroke();

        ctx.strokeStyle = `rgba(${white.r}, ${white.g}, ${white.b}, ${0.9})`;
        ctx.lineWidth = 8;
        buildClippedPath();
        ctx.stroke();
        ...
        ctx.strokeStyle = `rgba(${accent.r}, ${accent.g}, ${accent.b}, ${0.3 + 0.18 * effectiveIntensity})`;
        ctx.lineWidth = 1.5;
        ...
        const packetConfigs = [
            { sprite: AmbientViz.sprites?.glowWhite, size: 28, alpha: 0.82, speed: 0.08, offset: 0.12 },
            { sprite: AmbientViz.sprites?.glowOrange, size: 24, alpha: 0.55, speed: 0.06, offset: 0.58 },
        ];

        ctx.globalCompositeOperation = 'lighter';
        for (const packet of packetConfigs) {
            if (!packet.sprite) continue;
            const point = getPointAlongPolyline(points, (time * packet.speed + packet.offset) % 1);
            if (!point) continue;

            const size = packet.size + shimmer * 6;
            ctx.globalAlpha = packet.alpha;
            ctx.drawImage(packet.sprite, point.x - size / 2, point.y - size / 2, size, size);
        }
    ...
    renderRoundReview(ctx, deltaTime) {
    ...
    renderRoundHistory(ctx, deltaTime, isVizActive = false, excludeRound = null) {
        ...
        for (const round of rounds) {
            ...
            const effectiveIntensity = window.PathfindrMusicalRoutes?.historyIntensity(round.intensity,vizDimFactor,PathfindrAudio.state,rounds.length,isVizActive) ?? round.intensity*vizDimFactor;
            ElectricitySystem.renderElectrifiedEdges(ctx, screenEdges, round.color, effectiveIntensity, isActive);
            window.PathfindrMusicalRoutes?.network(ctx,round.musicalTree,GameState.map,()=>true,round.hotColor||round.color,isVizActive?.4:.9,Math.max(24,144/rounds.length));
            ...
            PathfindrRouteCinema.route(ctx,savedUserPoints,{r:255,g:184,b:105},performance.now()*.001,false,true);ctx.restore();
    ...
    renderPathHistory(ctx, deltaTime, paths, isVizActive = false) {
        ...
        for (const path of paths) {
            ...
            let effectiveIntensity = window.PathfindrMusicalRoutes?.historyIntensity(path.intensity,vizDimFactor,PathfindrAudio.state,paths.length,isVizActive) ?? path.intensity*vizDimFactor;
            ...
            if (optimalPoints && optimalPoints.length > 1) {
                ...
                window.PathfindrMusicalRoutes?.draw(ctx,optimalPoints,optimalColor,'history-optimal:'+paths.indexOf(path),false,2);
            }
            ...
            if (userPoints && userPoints.length > 1) {
                ...
                window.PathfindrMusicalRoutes?.draw(ctx,userPoints,path.midColor||path.color,'history-user:'+paths.indexOf(path),false,2);
            }
        }
...
const RoundHistory = {
    ...
    addRound(roundNumber, exploredEdges, optimalPath, userPath) {
        ...
        const exploredKeys=new Set(exploredEdges),tree=GameState.vizState.musicalTree;
        const musicalTree=tree?{max:tree.max,segments:tree.segments.filter(e=>exploredKeys.has(getCanonicalEdgeKey(e.from,e.to)))}:null;
...
const VisualizerHistory = {
    paths: [],
    pathIndex: 0,
    version: 0,        // Incremented whenever paths change (for renderer cache invalidation)
...
// =============================================================================
// VISUALIZER VIBE RENDERER - Cached, persistent, high-performance history layer
// =============================================================================

const VisualizerVibeRenderer = {
    coreCanvas: null,
    coreCtx: null,
    glowCanvas: null,
    glowCtx: null,
    ...
    clearCanvases() {
        if (this.coreCtx && this.width > 0 && this.height > 0) {
            this.coreCtx.clearRect(0, 0, this.width, this.height);
        }
        if (this.glowCtx && this.width > 0 && this.height > 0) {
    ...
    isMapStateSame(a, b) {
        if (!a || !b) return false;
        return (
            Math.abs(a.centerLat - b.centerLat) < 1e-7 &&
    ...
    canApproximateWithPanOffset(a, b) {
        if (!a || !b) return false;
        return (
    ...
    computePanOffset(snapshot) {
        if (!snapshot || !GameState.map) return { x: 0, y: 0 };
        const projected = GameState.map.project([snapshot.centerLng, snapshot.centerLat]);
    ...
    getAdaptiveStride(edgeCount, pathCount, isMoving) {
        let budget = this.maxEdgesPerPath;
        if (isMoving) budget = Math.floor(budget * 0.75);
        if (pathCount > 28) budget = Math.floor(budget * 0.85);
        if (pathCount > 45) budget = Math.floor(budget * 0.7);
        if (this.frameTimeEMA > 20) budget = Math.floor(budget * 0.52);
        else if (this.frameTimeEMA > 17) budget = Math.floor(budget * 0.72);
        else if (this.frameTimeEMA < 12) budget = Math.floor(budget * 1.2);
        return Math.max(1, Math.ceil(edgeCount / Math.max(1000, budget)));
    },

    getVibeColor(baseColor) {
        return baseColor || { r: 85, g: 223, b: 226 };
    },
    ...
    appendPath(path, isMoving = false) {
        ...
        for (const segment of (path.exploredSegments || [])) {
            ...
            if ((from.x < minX && to.x < minX) || (from.x > maxX && to.x > maxX)) continue;
            if ((from.y < minY && to.y < minY) || (from.y > maxY && to.y > maxY)) continue;

            segments.push({ from, to });
        }
        // Keep compatibility with living-network spark spawning logic.
        path.screenEdgesCache = segments;
        path.cacheValid = true;
        ...
        if ((path.optimalPathCoords && path.optimalPathCoords.length > 1) || (path.optimalPath && path.optimalPath.length > 1)) {
            ...
            path.vibeOptimalPoints = points;
            if (points.length > 1) {
                const hot = {r:85,g:223,b:226}; // Classic's algorithm filament.
                const glow = this.glowCtx;
                glow.globalCompositeOperation = 'lighter';
                ...
                const core = this.coreCtx;
                core.globalCompositeOperation = 'lighter';
                core.strokeStyle = `rgba(${hot.r}, ${hot.g}, ${hot.b}, 0.18)`;
                core.lineWidth = 3.3;
                core.beginPath();
    ...
    rebuildAll(paths, mapSnapshot, isMoving = false) {
        this.historyBlend = null; // Never retain screen-space imagery across camera changes.
        this.clearCanvases();
        for (const path of paths) {
            this.appendPath(path, isMoving);
        }
        this.lastMapSnapshot = mapSnapshot;
        this.lastHistoryVersion = VisualizerHistory.version;
        this.renderedPathCount = paths.length;
    ...
    sync(width, height) {
        ...
        const paths = VisualizerHistory.getPaths();
        const mapSnapshot = this.getMapSnapshot();
        const now = performance.now();
        const moving = GameState.map && GameState.map.isMoving ? GameState.map.isMoving() : false;
        ...
        const mapChanged = !this.isMapStateSame(mapSnapshot, this.lastMapSnapshot);
        const historyReset = VisualizerHistory.version < this.lastHistoryVersion || paths.length < this.renderedPathCount;
        const versionChanged = VisualizerHistory.version !== this.lastHistoryVersion;

        if (historyReset || this.lastHistoryVersion === -1 || !this.lastMapSnapshot) {
            this.rebuildAll(paths, mapSnapshot, moving);
            return;
        }
        ...
        if (mapChanged) {
            if (shouldUseLegacyMobileSync() &&
                ...
                const offset = this.computePanOffset(this.lastMapSnapshot);
                this.drawOffsetX = offset.x;
                this.drawOffsetY = offset.y;
        ...
        if (versionChanged) {
            // Crossfade only cached route layers, keeping labels/buildings sharp.
            const copy = source => {
                const canvas = document.createElement('canvas');
            ...
            this.historyBlend = {core: copy(this.coreCanvas), glow: copy(this.glowCanvas), started: performance.now()};
            for (let i = this.renderedPathCount; i < paths.length; i++) {
                this.appendPath(paths[i], moving);
            }
            this.renderedPathCount = paths.length;
            this.lastHistoryVersion = VisualizerHistory.version;
            this.lastMapSnapshot = mapSnapshot;
        ...
        if (this.pendingRebuild && (now - this.lastRebuildTime) >= this.rebuildIntervalMs) {
            this.rebuildAll(paths, mapSnapshot, moving);
        }
    },

    renderDepth(ctx, width, height, isVizActive) {
        if (!GameState.map) return;
        const focus = [];
        if (GameState.startMarkerLatLng) {
            const p = GameState.map.project([GameState.startMarkerLatLng.lng, GameState.startMarkerLatLng.lat]);
            if (isFinite(p.x) && isFinite(p.y)) focus.push(p);
        }
        if (GameState.endMarkerLatLng) {
            const p = GameState.map.project([GameState.endMarkerLatLng.lng, GameState.endMarkerLatLng.lat]);
            if (isFinite(p.x) && isFinite(p.y)) focus.push(p);
        }
        if (focus.length === 0) {
            focus.push({ x: width * 0.52, y: height * 0.46 });
        }

        ctx.save();
        ctx.globalCompositeOperation = 'screen';
        ctx.globalAlpha = 0.17 - 0.05 * Number(isVizActive);
        for (const p of focus) {
            const radius = Math.max(width, height) * 0.42;
            const g = ctx.createRadialGradient(p.x, p.y, 20, p.x, p.y, radius);
            g.addColorStop(0, 'rgba(160, 220, 255, 0.22)');
            g.addColorStop(0.45, 'rgba(90, 130, 255, 0.1)');
        ...
        ctx.globalAlpha = 0.22 - 0.06 * Number(isVizActive);
        const vignette = ctx.createRadialGradient(width * 0.5, height * 0.5, Math.min(width, height) * 0.18, width * 0.5, height * 0.5, Math.max(width, height) * 0.7);
        vignette.addColorStop(0, 'rgba(0, 0, 0, 0)');
        vignette.addColorStop(0.65, 'rgba(8, 10, 20, 0.18)');
        vignette.addColorStop(1, 'rgba(2, 4, 10, 0.35)');
        ctx.fillStyle = vignette;
        ctx.fillRect(0, 0, width, height);
        ctx.restore();
    },

    render(ctx, width, height, isVizActive = false) {
        this.noteFrame();
        this.sync(width, height);
        if (!this.coreCanvas || !this.glowCanvas) return;

        const breathe = 0.92 + Math.sin(performance.now() * 0.001 * 0.7) * 0.08;
        const drawX = -this.drawOffsetX;
        const drawY = -this.drawOffsetY;

        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        const blend = this.historyBlend;
        const t = blend ? (GameState.vizState.phase === 'settling' && GameState.vizState.active
        ...
        if (blend && t < 1) {
            ...
            ctx.drawImage(blend.glow, drawX, drawY, width, height);
            ctx.globalAlpha = coreAlpha * (1 - mix);
            ctx.drawImage(blend.core, drawX, drawY, width, height);
        } else this.historyBlend = null;
        ctx.globalAlpha = glowAlpha * mix;
        ctx.drawImage(this.glowCanvas, drawX, drawY, width, height);
        ctx.globalAlpha = coreAlpha * mix;
        ctx.drawImage(this.coreCanvas, drawX, drawY, width, height);
        ctx.translate(drawX, drawY);
        ctx.setLineDash([8, 10]);
        ctx.lineDashOffset = PathfindrMotion.reduced() ? 0 : -(performance.now() * .026) % 18;
        ctx.lineWidth = 3.3;
        ctx.strokeStyle = '#55dfe2';
        const paths = VisualizerHistory.getPaths();
        for (let i = 0; i < paths.length; i++) {
            const points = paths[i].vibeOptimalPoints;
            if (!points || points.length < 2) continue;
            ctx.globalAlpha = coreAlpha * (blend && i === paths.length - 1 ? mix : 1);
            drawSmoothPath(ctx, points); ctx.stroke();
        }
        ctx.restore();

        this.renderDepth(ctx, width, height, isVizActive);
    },
};

// =============================================================================
// VISUALIZER PHASE BLEND - Smooth crossfade between visualizer phases
// =============================================================================

const VisualizerPhaseBlend = {
    canvas: null,
    ctx: null,
    width: 0,
    height: 0,
    ...
    capture(options = {}) {
        const source = GameState.vizCanvas;
        if (!source || source.width <= 0 || source.height <= 0) return;

        this.ensureBuffer(source.width, source.height);
        this.ctx.clearRect(0, 0, source.width, source.height);
        this.ctx.drawImage(source, 0, 0, source.width, source.height);

        this.durationMs = Math.max(120, options.durationMs || 900);
        this.maxAlpha = Math.max(0, Math.min(1, options.maxAlpha ?? 0.7));
    ...
    render(ctx, width, height) {
        ...
        const eased = 1 - Math.pow(progress, 1.45);
        const alpha = this.maxAlpha * eased * eased;
        if (alpha <= 0.01) {
            this.active = false;
            return;
        }
        ...
        ctx.drawImage(this.canvas, 0, 0, this.width, this.height, 0, 0, width, height);
        ctx.restore();
    },
};
...
const GameState = {
    ...
    visualizerState: {
        ...
        loopTimeout: null,
        loopRunning: false,
        delayBetweenRuns: 120,  // brief breath; the history keeps animating
        runId: 0,                // Monotonic token to cancel stale async work
    },
...
function initCanvases() {
    ...
    GameState.previewCanvas = document.getElementById('preview-canvas');
...
/**
 * Setup path input handling on top of the MapLibre surface.
 * The draw canvas stays visual-only; clicks/touches land on the map beneath it.
 */
function setupMobileTouchHandling() {
    const canvas = GameState.drawCanvas;
    if (!canvas || !GameState.map) return;
    const inputSurface = GameState.map.getCanvasContainer();

    // Make canvas non-interactive - let pointer/touch events reach MapLibre
    ...
    // Desktop click/tap fallback via MapLibre click events
    ...
    inputSurface.addEventListener('touchstart', (e) => {
        ...
        GameState.touchPreviewActive = true;
    ...
    inputSurface.addEventListener('touchend', (e) => {
        ...
        if (touch && shouldHandlePathInput()) {
            GameState.suppressNextMapClickUntil = Date.now() + 400;
            const { lat, lng } = getLatLngFromClient(touch.clientX, touch.clientY);
            commitPathPoint(lat, lng);
    ...
    GameState.pathPreviewHandlersAdded = true;
}
    ...
    for (const element of data.elements) {
        if (element.type === 'way' && element.nodes) {
            ...
            for (let i = 0; i < element.nodes.length - 1; i++) {
                ...
                const edgeKey = getCanonicalEdgeKey(nodeA, nodeB);
                ...
                const edgeObj = {
                    ...
                    layer: Number.isFinite(Number(tags.layer)) ? Number(tags.layer) : 0,
                };
...
function startGame() {
    if(GameState.gameMode==='competitive')requestVisualizerFullscreen();
...
async function nextRound() {
    ...
    try {
        ...
        if(GameState.currentRound>=CONFIG.totalRounds&&GameState.continuousPlay.enabled){
            await transitionToNextCity();
            return;
        }
        ...
        document.getElementById('city-transition')?.classList.add('hidden');
        if(GameState.gameStarted&&GameController.phase!==GamePhase.MENU){
            GameController.enterPhase(GamePhase.RESULTS);showResults();
            document.getElementById('next-round-btn').querySelector('span').textContent='Retry next city';
            showToast('Next city is not ready yet. Your result is saved—tap Retry next city.',6000);
        }
    } finally {
...
function selectRandomEndpoints(options = {}) {
    const selection = createEndpointSelection(options);
    let step;
    do { step = selection.next(); } while (!step.done);
    return step.value;
}

function* createEndpointSelection(options = {}) {
    ...
    const getRouteShapeMetrics = function* (candidate) {
        const cached = routeShapeCache.get(candidate);
        if (cached) return cached;

        routeSearchCount++;
        const path = (yield* createAStarSearch(candidate.startNode, candidate.endNode, { trackExplored: false })).path;
    ...
    for (const candidate of shapeCandidates) {
        const shape = yield* getRouteShapeMetrics(candidate);
    ...
    const preparedRoute = yield* createAStarSearch(finalSelected.startNode, finalSelected.endNode);
    routeSearchCount++;
...
function getActivePathAnchorNode() {
    if (GameState.userPathNodes.length > 0) {
        return GameState.userPathNodes[GameState.userPathNodes.length - 1];
    }

    return GameState.startNode || null;
}

function getLatLngFromClient(clientX, clientY) {
...
function getLatLngFromEvent(e) {
    return getLatLngFromClient(e.clientX, e.clientY);
}

function buildNodePathCoords(nodeIds) {
    return (nodeIds || [])
...
function clearSnapPreview(options = {}) {
    ...
    GameState.snapPreview.inputType = null;
    GameState.snapPreview.signature = null;
...
function buildPreviewPathCoords(anchorNodeId, snapTarget) {
    ...
    const edgeKey = getCanonicalEdgeKey(snapTarget.fromNode, snapTarget.toNode);
    if (anchorPos.virtual && anchorPos.edgeKey === edgeKey) {
        return [
            { lat: anchorPos.lat, lng: anchorPos.lng },
            { lat: snapTarget.point.lat, lng: snapTarget.point.lng },
        ];
    }

    if (anchorNodeId === snapTarget.fromNode || anchorNodeId === snapTarget.toNode) {
        return [
            { lat: anchorPos.lat, lng: anchorPos.lng },
            { lat: snapTarget.point.lat, lng: snapTarget.point.lng },
        ];
    ...
    const endpointOptions = [snapTarget.fromNode, snapTarget.toNode]
        .map((endpointNodeId) => {
            ...
            const segmentToSnap = haversineDistance(
                endpointPos.lat,
                endpointPos.lng,
                snapTarget.point.lat,
                snapTarget.point.lng
            );

            return {
                totalDistance: calculateNodePathDistance(pathNodes) + segmentToSnap,
                coords: [
                    ...nodePathCoords,
                    { lat: snapTarget.point.lat, lng: snapTarget.point.lng },
                ],
...
function updateSnapPreview(lat, lng, inputType = 'mouse') {
    ...
    const anchorNodeId = anchorContext?.anchorNodeId || getActivePathAnchorNode();
    const anchorPos = anchorContext?.anchorPos || (anchorNodeId ? GameState.nodes.get(anchorNodeId) : null);
    if (!anchorPos) {
        clearSnapPreview();
        return null;
    }
    ...
    const previewPoint = snapTarget.type === 'node'
        ? GameState.nodes.get(snapTarget.nodeId)
        : snapTarget.point;
    ...
    const pathCoords = buildPreviewPathCoords(anchorNodeId, snapTarget);
    ...
    if (GameState.gameMode !== 'explorer') {
        const routedDistance = calculateCoordPathDistance(pathCoords);
        const maxRouteDistance = getMaxRoutedSegmentDistanceKm(clickDistance, snapTarget, anchorContext);
    ...
    const previewSignature = snapTarget.type === 'node'
        ? `node:${snapTarget.nodeId}`
        : `edge:${snapTarget.fromNode}:${snapTarget.toNode}:${previewPoint.lat.toFixed(4)}:${previewPoint.lng.toFixed(4)}`;
    const shouldPlayPreviewCue = GameState.snapPreview.signature !== previewSignature;

    GameState.snapPreview.active = true;
    GameState.snapPreview.point = { lat: previewPoint.lat, lng: previewPoint.lng };
    GameState.snapPreview.pathCoords = pathCoords;
    GameState.snapPreview.inputType = inputType;
    GameState.snapPreview.signature = previewSignature;

    if (shouldPlayPreviewCue && typeof SoundEngine !== 'undefined') {
        SoundEngine.previewLock(previewSignature);
    }

    redrawUserPath();
    return GameState.snapPreview;
}
...
function commitPathPoint(lat, lng, options = {}) {
    ...
    GameState.userDrawnPoints.push({ lat, lng });
    clearSnapPreview({ redraw: false });

    if (addPointToUserPath(lat, lng)) {
        redrawUserPath();
        if(!PathfindrTrace.active&&shouldHandlePathInput())nudgeRouteHeadIntoView();
        SoundEngine.init();
        if (!options.quiet) {
            SoundEngine.click();
...
function handlePathClick(e) {
    ...
    commitPathPoint(point.lat, point.lng);
}
...
function redrawUserPath() {
    ...
    // Always render click radius indicator during PLAYING phase, even before the first click
    if (GameController.phase === GamePhase.PLAYING && getActivePathAnchorNode()) {
    ...
    if (points.length >= 2) {
        ...
        const flicker = 0.88 + PathfindrAudio.state.energy * .08;

        // Add subtle wobble to points for "hand-drawn" feel
        const wobblePoints = points; // Route geometry is stable; only its light moves.
        ...
        ctx.strokeStyle = `rgba(${uc.r}, ${Math.min(255, uc.g + 20)}, ${uc.b}, ${0.16 * flicker})`;
        ctx.lineWidth = 7;
        drawWobblyPath(ctx, wobblePoints);
        ctx.stroke();

        // Inner glow - brightest
        ctx.strokeStyle = `rgba(${uc.r}, ${Math.min(255, uc.g + 40)}, ${uc.b}, ${0.12 * flicker})`;
        ctx.lineWidth = 4;
        drawWobblyPath(ctx, wobblePoints);
        ctx.stroke();
        ...
        ctx.setLineDash([20,9,2,9]);
        ctx.lineDashOffset=PathfindrMotion.reduced()?0:-(time*34)%40;
        ...
        window.PathfindrMusicalRoutes?.draw(ctx,points,uc,'drawing',true);
    }
...
function renderClickRadiusIndicator(ctx, time) {
    const lastNodeId = getActivePathAnchorNode();
...
function renderSnapPreview(time) {
    const ctx = GameState.previewCtx;
    const preview = GameState.snapPreview;
    if (!ctx || !preview.active || !preview.point || !GameState.map) return;

    const previewPoints = preview.pathCoords
        .map((pos) => GameState.map.project([pos.lng, pos.lat]));

    const previewColor = { r: 115, g: 248, b: 255 };
    const accentColor = { r: 255, g: 90, b: 214 };
    const pulse = 0.65 + 0.35 * Math.sin(time * 4.6);
    const shimmer = 0.55 + 0.45 * Math.sin(time * 8.4 + 0.8);
    ...
    if (previewPoints.length >= 2) {
        ...
        ctx.globalCompositeOperation = 'source-over';
        for (let i = 0; i <= 10; i++) {
            const point = getPointAlongPolyline(previewPoints, i / 10);
            if (!point) continue;
            const corridor = ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, 42);
            corridor.addColorStop(0, 'rgba(4, 3, 12, 0.18)');
            corridor.addColorStop(0.45, 'rgba(4, 3, 12, 0.10)');
            corridor.addColorStop(1, 'rgba(4, 3, 12, 0)');
        ...
        ctx.globalCompositeOperation = 'lighter';
        ctx.strokeStyle = `rgba(${previewColor.r}, ${previewColor.g}, ${previewColor.b}, ${0.1 + 0.12 * pulse})`;
        ctx.lineWidth = 16;
        drawWobblyPath(ctx, previewPoints);
        ctx.stroke();

        ctx.strokeStyle = `rgba(${accentColor.r}, ${accentColor.g}, ${accentColor.b}, ${0.05 + 0.08 * shimmer})`;
        ctx.lineWidth = 10;
        drawWobblyPath(ctx, previewPoints);
        ctx.stroke();
        ...
        ctx.strokeStyle = `rgba(${previewColor.r}, ${previewColor.g}, ${previewColor.b}, 0.98)`;
        ctx.lineWidth = 2.3;
        ctx.setLineDash([14, 10]);
        ctx.lineDashOffset = -time * 82;
        drawWobblyPath(ctx, previewPoints);
        ctx.stroke();

        ctx.strokeStyle = `rgba(${accentColor.r}, ${accentColor.g}, ${accentColor.b}, 0.88)`;
        ctx.lineWidth = 1.35;
        ctx.setLineDash([3, 16]);
        ctx.lineDashOffset = time * 64;
        drawWobblyPath(ctx, previewPoints);
        ctx.stroke();

        ctx.strokeStyle = `rgba(255, 255, 255, ${0.52 + 0.28 * shimmer})`;
        ctx.lineWidth = 1.1;
        ctx.setLineDash([1.5, 20]);
        ctx.lineDashOffset = -time * 120;
        drawWobblyPath(ctx, previewPoints);
        ctx.stroke();

        const spriteTrail = [
            { sprite: AmbientViz.sprites?.glowWhite, size: 26, alpha: 0.92, speed: 0.54, offset: 0.00 },
            { sprite: AmbientViz.sprites?.glowCyan,  size: 22, alpha: 0.82, speed: 0.41, offset: 0.22 },
            { sprite: AmbientViz.sprites?.glowPink,  size: 20, alpha: 0.62, speed: 0.31, offset: 0.47 },
            { sprite: AmbientViz.sprites?.glowWhite, size: 18, alpha: 0.52, speed: 0.22, offset: 0.74 },
        ];

        ctx.globalCompositeOperation = 'lighter';
        for (const trail of spriteTrail) {
            if (!trail.sprite) continue;
            const point = getPointAlongPolyline(previewPoints, (time * trail.speed + trail.offset) % 1);
            if (!point) continue;
            const size = trail.size + pulse * 7;
            ctx.globalAlpha = trail.alpha;
            ctx.drawImage(trail.sprite, point.x - size / 2, point.y - size / 2, size, size);
        }
        ctx.globalAlpha = 1;
        ctx.restore();
    }

    const targetScreen = GameState.map.project([preview.point.lng, preview.point.lat]);
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';

    const targetDim = ctx.createRadialGradient(targetScreen.x, targetScreen.y, 0, targetScreen.x, targetScreen.y, 52);
    targetDim.addColorStop(0, 'rgba(4, 3, 12, 0.20)');
    targetDim.addColorStop(0.5, 'rgba(4, 3, 12, 0.10)');
    targetDim.addColorStop(1, 'rgba(4, 3, 12, 0)');
    ctx.globalCompositeOperation = 'source-over';
    ctx.fillStyle = targetDim;
    ctx.beginPath();
    ctx.arc(targetScreen.x, targetScreen.y, 52, 0, Math.PI * 2);
    ctx.fill();

    ctx.globalCompositeOperation = 'lighter';
    if (AmbientViz.sprites?.glowWhite) {
        const size = 58 + pulse * 12;
        ctx.globalAlpha = 0.7;
        ctx.drawImage(AmbientViz.sprites.glowWhite, targetScreen.x - size / 2, targetScreen.y - size / 2, size, size);
    }
    if (AmbientViz.sprites?.glowCyan) {
        const size = 46 + pulse * 10;
        ctx.globalAlpha = 0.82;
        ctx.drawImage(AmbientViz.sprites.glowCyan, targetScreen.x - size / 2, targetScreen.y - size / 2, size, size);
    }
    if (AmbientViz.sprites?.glowPink) {
        const size = 34 + shimmer * 8;
        ctx.globalAlpha = 0.56;
        ctx.drawImage(AmbientViz.sprites.glowPink, targetScreen.x - size / 2, targetScreen.y - size / 2, size, size);
    }

    ctx.beginPath();
    ctx.arc(targetScreen.x, targetScreen.y, 22 + pulse * 5, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(${previewColor.r}, ${previewColor.g}, ${previewColor.b}, ${0.12 + 0.08 * pulse})`;
    ctx.fill();

    ctx.beginPath();
    ctx.arc(targetScreen.x, targetScreen.y, 17 + pulse * 3, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(255, 255, 255, ${0.5 + 0.25 * shimmer})`;
    ctx.lineWidth = 1.75;
    ctx.setLineDash([6, 8]);
    ...
    ctx.beginPath();
    ctx.arc(targetScreen.x, targetScreen.y, 10 + pulse * 2, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(${accentColor.r}, ${accentColor.g}, ${accentColor.b}, ${0.8 + 0.15 * shimmer})`;
    ctx.lineWidth = 2.3;
    ctx.stroke();

    ctx.beginPath();
    ctx.arc(targetScreen.x, targetScreen.y, 6.5, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(${previewColor.r}, ${previewColor.g}, ${previewColor.b}, 0.95)`;
    ctx.lineWidth = 1.8;
    ctx.stroke();

    ctx.beginPath();
    ctx.arc(targetScreen.x, targetScreen.y, 3.8, 0, Math.PI * 2);
    ...
    ctx.lineTo(targetScreen.x - 6, targetScreen.y);
    ctx.moveTo(targetScreen.x + 6, targetScreen.y);
    ctx.lineTo(targetScreen.x + 18, targetScreen.y);
    ctx.moveTo(targetScreen.x, targetScreen.y - 18);
    ctx.lineTo(targetScreen.x, targetScreen.y - 6);
    ctx.moveTo(targetScreen.x, targetScreen.y + 6);
...
const AStarMusicalTrees = new WeakMap();
function runAStar(startNode, endNode, options = {}) {
    const search = createAStarSearch(startNode, endNode, options);
    let step;
    do { step = search.next(); } while (!step.done);
    return step.value;
}

// Same algorithm/results for both modes; the autonomous presentation yields
// between small batches so large graphs cannot monopolize the main thread.
function* createAStarSearch(startNode, endNode, options = {}) {
    ...
    if (frontier) AStarMusicalTrees.set(exploredOrder,cameFrom);
...
async function submitRoute() {
    ...
    // Camera anticipation replaces the old stationary pause.
    ...
    const revealCamera=createRouteRevealCamera(path);
    await revealCamera.begin();
    if (!stillCurrent() || GameController.phase !== GamePhase.PLAYING) {revealCamera.dispose();return;}
    ...
    // Start visualization
    try {
        await runEpicVisualization(explored, path, stillCurrent, {onFrame:progress=>revealCamera.update(progress)});
    } finally {revealCamera.dispose();}
...
async function runEpicVisualization(explored, path, shouldContinue = null, options = {}) {
    ...
    viz.musicalTree=window.PathfindrMusicalRoutes?.tree(AStarMusicalTrees.get(explored)||new Map(),path[0],GameState.nodes,haversineDistance);
    let cursor = 0, settled = false;
    const completed = await PathfindrMotion.animate(duration, progress => {
        const t = progress * duration;
        viz.sequenceMs = t;
        options.onFrame?.(Math.min(1,t/exploreMs));
        ...
        if (t >= settleStart) {
            viz.settleProgress = Math.min(1, (t - settleStart) / settleMs);
            if (!settled) {
                settled = true;
...
function drawAmbientRoads(ctx, time, width, height, opacity = 1) {
    opacity *= getAmbientRoadVisibility();
    if (!opacity) return;
...
function getCurrentVizTheme() {
    let colorIndex;
    if (GameState.gameMode === 'visualizer') {
        colorIndex = GameState.visualizerState.themeIndex ?? VisualizerHistory.pathIndex;
    } else if (GameState.gameMode === 'explorer') {
...
function getSearchCoolingGain(now=performance.now()){
    const viz=GameState.vizState;
    if(viz.phase!=='settling')return 1;
    if(GameState.gameMode==='visualizer'){
        const t=viz.settleProgress ?? Math.min(1,Math.max(0,(now-viz.settleStartTime)/(viz.settleDuration||2200)));
        return 1-t*t*(3-2*t);
    }
...
function renderVisualization(options = {}) {
    ...
    if (scene.isExplorerMode && ExplorerHistory.hasHistory()) {
        ...
        const elapsed = Math.max(0, performance.now() - (viz.settleStartTime || 0));
        const t = viz.phase === 'settling' ? (viz.settleProgress ?? Math.min(1, elapsed / (viz.settleDuration || 2200))) : 0;
        const entry = Math.min(1, Math.max(0, (performance.now() - GameState.visualizerState.searchStartedAt) / 500));
        VisualizerVibeRenderer.render(ctx, width, height, (1 - t * t * (3 - 2 * t)) * entry * entry * (3 - 2 * entry));
    ...
    if (viz.phase === 'settling') {
        ...
        const settleDuration = viz.settleDuration || (isVisualizerMode ? 1150 : 800);
        settleProgress = isVisualizerMode ? (viz.settleProgress ?? Math.min(1, elapsed / settleDuration)) : Math.min(1, elapsed / settleDuration);

        // Ease-out curve for smooth deceleration
        const easeOut = 1 - Math.pow(1 - settleProgress, 2);

        // VISUALIZER MODE: Blend from live heat into persistent history instead of hard-switch.
        if (isVisualizerMode) {
            const holdRatio = 0.16;
            const blendT = Math.max(0, (settleProgress - holdRatio) / (1 - holdRatio));
    ...
    const settlingDecay = viz.phase === 'settling'
        ? (isVisualizerMode
            ? Math.pow(baseDecay, 1 + settleProgress * 4.5)
            : Math.pow(baseDecay, 1 + settleProgress * 2))
        : baseDecay;
    // Keep floor at minimum 20% during settling so heat blends into ambient
    const heatFloor = viz.phase === 'settling'
        ? (isVisualizerMode
            ? Math.max(0.04, CONFIG.viz.heatFloor * (1 - settleProgress * 0.98))
    ...
    if (!isVisualizerMode && viz.phase === 'settling' && ambientOpacity > 0.1) {
        ctx.save();
    ...
    let roadOpacity = 1;
    if (viz.phase === 'path') {
        const t = Math.min(1, Math.max(0, (now - viz.optimalRevealStartTime) / 420));
        roadOpacity = isVisualizerMode ? 1 - (1 - optimalDimMin) * t * t * (3 - 2 * t) : optimalDimMin;
    } else if (viz.phase === 'settling' && isVisualizerMode) {
        roadOpacity = optimalDimMin + (1 - optimalDimMin) * settleProgress * settleProgress * (3 - 2 * settleProgress);
    } else if(viz.phase==='settling'&&!isVisualizerMode){
    ...
    if (!scene.isResultsPhase && (viz.phase === 'path' || viz.phase === 'complete' || viz.phase === 'settling')) {
        if (viz.phase === 'settling') {
            ctx.save();
            ctx.globalAlpha = isVisualizerMode ? heatOpacity : 1;
        }
        drawOptimalPath(ctx);
        if (viz.phase === 'settling') {
            ctx.restore();
    ...
    window.PathfindrMusicalRoutes?.network(ctx,viz.musicalTree,GameState.map,(a,b)=>viz.edgeHeat.has(getCanonicalEdgeKey(a,b)),getCurrentVizTheme().hot,getSearchCoolingGain(now));
...
function drawOptimalPath(ctx) {
    ...
    const theme = getCurrentVizTheme();
    const oc = CONFIG.color.getOptimalPathColor();
...
function addPointToUserPath(lat, lng) {
    ...
    if (microPath.length === 0) {
        ...
        for (const nodeId of newNodes) {
            if(PathfindrTrace.active&&GameState.userPathNodes.at(-2)===nodeId){GameState.userPathNodes.pop();continue;}
...
function normalizeLayerValue(layer) {
    const parsed = Number(layer);
    return Number.isFinite(parsed) ? parsed : 0;
}

function getEdgeStructureSignature(edge) {
    if (!edge) return 'surface:0';

    if (edge.tunnel) return `tunnel:${normalizeLayerValue(edge.layer)}`;
    if (edge.bridge) return `bridge:${normalizeLayerValue(edge.layer)}`;
    return `surface:${normalizeLayerValue(edge.layer)}`;
}

function getHighwaySnapPenalty(highway) {
    if (!highway) return 0;
    return SNAP_HIGHWAY_PENALTIES[highway] || 0;
}

function getAnchorSnapContext() {
    const anchorNodeId = getActivePathAnchorNode();
    ...
    if (GameState.userPathNodes.length >= 2 && GameState.userPathNodes[GameState.userPathNodes.length - 1] === anchorNodeId) {
        previousNodeId = GameState.userPathNodes[GameState.userPathNodes.length - 2];
    }

    let edgeKey = null;
    if (anchorPos.virtual && anchorPos.edgeKey) {
        edgeKey = anchorPos.edgeKey;
    } else if (previousNodeId != null) {
        edgeKey = getCanonicalEdgeKey(previousNodeId, anchorNodeId);
    }

    const edge = edgeKey ? GameState.edgeLookup.get(edgeKey) : null;
    let forward = null;

    if (previousNodeId != null) {
        const previousPos = GameState.nodes.get(previousNodeId);
        if (previousPos) {
            const scale = Math.cos(anchorPos.lat * Math.PI / 180);
            const dx = (anchorPos.lng - previousPos.lng) * scale;
            const dy = anchorPos.lat - previousPos.lat;
    ...
    return {
        ...
        edgeKey,
        edge,
        structure: getEdgeStructureSignature(edge),
        forward,
    };
}

function scoreSnapEdgeCandidate(edge, distanceMeters, snappedPoint, anchorContext) {
    let score = distanceMeters;
    if (!edge) return score;

    const anchorEdge = anchorContext?.edge;
    const candidateEdgeKey = getCanonicalEdgeKey(edge.from, edge.to);
    const candidateStructure = getEdgeStructureSignature(edge);
    const sameStructure = !!anchorEdge && candidateStructure === anchorContext.structure;
    const sameName = !!anchorEdge?.name && !!edge.name && anchorEdge.name === edge.name;
    const sameHighway = !!anchorEdge?.highway && anchorEdge.highway === edge.highway;

    if (candidateEdgeKey === anchorContext?.edgeKey) score -= 34;
    if (sameName) score -= 18;
    if (sameHighway) score -= 8;
    if (sameStructure && candidateStructure !== 'surface:0') score -= 16;

    let highwayPenalty = getHighwaySnapPenalty(edge.highway);
    if (sameName || sameHighway || candidateEdgeKey === anchorContext?.edgeKey) {
        highwayPenalty *= 0.35;
    }
    score += highwayPenalty;

    if (anchorContext?.forward && snappedPoint) {
        const scale = Math.cos(anchorContext.anchorPos.lat * Math.PI / 180);
        const dx = (snappedPoint.lng - anchorContext.anchorPos.lng) * scale;
        const dy = snappedPoint.lat - anchorContext.anchorPos.lat;
        const length = Math.sqrt((dx * dx) + (dy * dy));

        if (length > 0) {
            const alignment = ((dx / length) * anchorContext.forward.x) + ((dy / length) * anchorContext.forward.y);
            if (alignment >= 0) {
                score -= alignment * 10;
            } else {
                score += Math.abs(alignment) * 16;
            }
        }
    }
...
function getSnapEdgeMeta(snapTarget) {
    if (!snapTarget || snapTarget.type !== 'edge') return null;
    return GameState.edgeLookup.get(getCanonicalEdgeKey(snapTarget.fromNode, snapTarget.toNode)) || null;
}
...
function getMaxRoutedSegmentDistanceKm(clickDistanceKm, snapTarget, anchorContext) {
    ...
    const anchorEdge = anchorContext?.edge;
    const sameNamedRoad = !!anchorEdge?.name && !!snapEdge?.name && anchorEdge.name === snapEdge.name;
    const sameStructuredRoad = !!anchorEdge && !!snapEdge &&
        getEdgeStructureSignature(anchorEdge) === getEdgeStructureSignature(snapEdge) &&
        getEdgeStructureSignature(snapEdge) !== 'surface:0';

    if (sameNamedRoad || sameStructuredRoad) {
        maxRouteDistanceKm = Math.max(maxRouteDistanceKm, maxDistanceKm * 4.5);
    } else if (snapEdge?.highway?.endsWith('_link')) {
        maxRouteDistanceKm = Math.max(maxRouteDistanceKm, maxDistanceKm * 3.75);
    }

    return Math.max(maxRouteDistanceKm, clickDistanceKm + 0.12);
}

function upsertEdgeNeighbor(nodeId, neighborId, weight) {
    if (!GameState.edges.has(nodeId)) GameState.edges.set(nodeId, []);

    const neighbors = GameState.edges.get(nodeId);
    const existing = neighbors.find((edge) => edge.neighbor === neighborId);
    if (existing) {
        existing.weight = Math.min(existing.weight, weight);
        return;
    }

    neighbors.push({ neighbor: neighborId, weight });
}
...
function findReusableVirtualNode(point, edgeKey) {
    const reuseThresholdKm = 0.005; // 5 meters keeps repeated clicks on the same spot stable

    for (const nodeId of GameState.virtualNodeIds) {
        const nodePos = GameState.nodes.get(nodeId);
        if (!nodePos?.virtual || nodePos.edgeKey !== edgeKey) continue;

        const dist = haversineDistance(point.lat, point.lng, nodePos.lat, nodePos.lng);
        if (dist <= reuseThresholdKm) {
            return nodeId;
        }
    }
...
function findNearestNodeWithDist(lat, lng, options = {}) {
    ...
    for (const nodeId of nodeIds) {
        ...
        const dist = haversineDistance(lat, lng, pos.lat, pos.lng) * 1000; // meters
        if (dist < nearestDist) {
            nearestDist = dist;
            nearestNode = nodeId;
        }
...
function findNearestEdgePoint(lat, lng, maxDistanceMeters, options = {}) {
    const { anchorContext = null } = options;
    const originLat = lat;
    const originLng = lng;
...
function findSnapTarget(lat, lng) {
    ...
    if(anchorContext){
        ...
        for(const candidate of candidates){
            ...
            if(PathfindrTrace.active&&window.PathfindrTraceGuide){
                const project=p=>GameState.map.project([p.lng,p.lat]);
                const tail=GameState.userPathNodes.slice(-40).map(id=>GameState.nodes.get(id)).filter(Boolean).map(project);
                if(!PathfindrTraceGuide.allows(tail,coords.map(project),project({lat,lng}),PathfindrTrace.pointerType==='touch'))continue;
            }
            return candidate;
        }
        return null;
    ...
    const nodeLockMeters = Math.min(14, Math.max(8, maxSnapDist * 0.12));
    if (nearestNode.nodeId && nearestNode.distance <= nodeLockMeters) {
...
function createVirtualNode(point, fromNode, toNode) {
    ...
    connectNodesBidirectionally(nodeId, fromNode, weightA);
    connectNodesBidirectionally(nodeId, toNode, weightB);

    // Virtual points on the same base edge must connect directly; otherwise
    // repeated clicks along a long street can detour via an endpoint and get rejected.
    for (const existingNodeId of GameState.virtualNodeIds) {
        const existingNode = GameState.nodes.get(existingNodeId);
        if (!existingNode?.virtual || existingNode.edgeKey !== edgeKey) continue;

        const directWeight = haversineDistance(point.lat, point.lng, existingNode.lat, existingNode.lng);
        connectNodesBidirectionally(nodeId, existingNodeId, directWeight);
    }
...
function initSplashScreen() {
    ...
    // The lobby is already visible in the initial HTML. Never hide it while
    // the runtime, soundtrack, city reserve or optional intro video loads.
    if (splashScreen.classList.contains('splash-show-welcome')) {
...
async function navigateToCity(city) {
    ...
    const runId = GameState.visualizerState.active
        ? VisualizerTimeline.createRunId()
        : GameState.visualizerState.runId;
    ...
    try {
        ...
        if (GameState.visualizerState.active && !VisualizerTimeline.isRunActive(runId)) return;
...
function enterFullscreenMode() {
    if(document.fullscreenElement||document.webkitFullscreenElement)exitVisualizerFullscreen();
    else requestVisualizerFullscreen();
}
...
function requestVisualizerFullscreen() {
    ...
    if (document.fullscreenEnabled && !document.fullscreenElement) {
        ...
        // Safari fallback
        try{const result=elem.webkitRequestFullscreen();result?.catch?.(()=>{});}catch{}
...
document.addEventListener('fullscreenchange', () => {
    ...
    if(button){const active=!!document.fullscreenElement;button.setAttribute('aria-pressed',String(active));button.title=active?'Exit fullscreen':'Enter fullscreen';}
    if (StreamConfig.enabled && StreamConfig.noEscExit) return;
...
document.addEventListener('keydown',e=>{
    if(e.key.toLowerCase()!=='f'||e.repeat||e.ctrlKey||e.metaKey||e.altKey||e.target?.closest?.('input,textarea,select,[contenteditable="true"]'))return;
...
const VisualizerTimeline = {
    config: {
        ...
        markerGapMs: 180,
        postDropHoldMs: 160,
        mapSettleTimeoutMs: 600,
    },

    createRunId() {
        GameState.visualizerState.runId += 1;
        return GameState.visualizerState.runId;
    },
    ...
    isRunActive(runId) {
        return GameState.visualizerState.active &&
            GameState.gameMode === 'visualizer' &&
            GameState.visualizerState.runId === runId;
    },

    clearHeroState() {
        const mapContainer = document.getElementById('map-container');
        if (mapContainer) mapContainer.classList.remove('visualizer-hero-active');
        if (GameState.startMarkerEl) {
            GameState.startMarkerEl.classList.remove('viz-marker-hidden', 'viz-marker-pop');
        }
        if (GameState.endMarkerEl) {
            GameState.endMarkerEl.classList.remove('viz-marker-hidden', 'viz-marker-pop');
        }
    },

    waitForMapSettle(timeoutMs = this.config.mapSettleTimeoutMs) {
        return new Promise(resolve => {
            const map = GameState.map;
            if (!map) {
                resolve();
    ...
    async playHeroIntro(runId) {
        ...
        const startEl = GameState.startMarkerEl;
        const endEl = GameState.endMarkerEl;
        if (!startEl || !endEl) {
            this.clearHeroState();
            return false;
        }
...
async function runVisualizerLoop() {
    ...
    const runId = VisualizerTimeline.createRunId();
    ...
    try {
        ...
        // Cinematic marker intro before each run
        const introOk = await VisualizerTimeline.playHeroIntro(runId);
        if (!introOk) return;

        // Run the A* visualization
        await runVisualizerAStar(runId);
        ...
        if (GameState.visualizerState.currentVisualization >= GameState.visualizerState.maxPerCity) {
            GameState.visualizerState.currentVisualization = 0;
            await loadNextVisualizerCity(runId);
        }
        ...
        GameState.visualizerState.loopTimeout = setTimeout(
            () => {
                // Prevent stale scheduled callbacks from previous runs.
...
async function runVisualizerAStar(runId) {
    ...
    GameState.exploredNodes = result.explored;

    if(GameState.gameMode==='visualizer')window.PathfindrVisualizerCamera?.follow(GameState.map,result.path,result.explored,GameState.nodes);
    ...
    if (result.path.length > 0) {
        ...
        await runEpicVisualization(
            result.explored,
            result.path,
            () => VisualizerTimeline.isRunActive(runId) && GameController.phase === GamePhase.VISUALIZING,
            {
                onBeforeSettling: () => {
                    if (VisualizerTimeline.isRunActive(runId)) {
                        commitToHistory();
        ...
        // Visualizer mode does not call score flow, so trigger completion cue here.
        if (SoundEngine.initialized) {
            SoundEngine.pathFound();
            void SoundEngine.fadeOutScanning(250);
        }
...
async function loadNextVisualizerCity(runId) {
    if (!VisualizerTimeline.isRunActive(runId)) return;
...
async function showExplorerComparison() {
    const graph=GameState.roadGraphVersion;
    const valid=()=>GameState.gameMode==='explorer'&&GameState.roadGraphVersion===graph&&
    ...
    if (result.path.length > 0) {
        GameState.optimalPath=result.path;GameState.exploredNodes=result.explored;
        const revealCamera=createRouteRevealCamera(result.path);
        await revealCamera.begin();
        if(!valid()){revealCamera.dispose();return;}
        // Enter VISUALIZING phase
        GameController.enterPhase(GamePhase.VISUALIZING);

        try {await runEpicVisualization(result.explored, result.path,valid,{onFrame:progress=>revealCamera.update(progress)});}
        finally {revealCamera.dispose();}
        if(!valid())return;
...
async function updateChallengeButton() {
    ...
    if (!PathfindrAuth.client) {
        ...
        desc.textContent = 'Challenges unavailable. Classic is ready.';
        return;
    }
...
async function transitionToNextCity() {
    ...
    const transitionId=GameState.cityTransitionId=(GameState.cityTransitionId||0)+1;
    const current=()=>GameState.cityTransitionId===transitionId&&GameState.continuousPlay.enabled&&GameState.roadGraphVersion===transitionGraphVersion;
    GameState.map.stop();
    ...
    if(cityName)cityName.textContent='Preparing next city…';
    ...
    if(!usedPreloadedCity||!usedPreloadedData?.elements?.some(e=>e.type==='way'&&e.nodes?.length>1))throw Error('Next city roads unavailable');
    ...
    // Wait before committing: an exit or failed preparation must preserve the completed city.
    await new Promise(resolve=>setTimeout(resolve,PathfindrMotion.reduced()?0:350));
    if(!current()){if(overlay)overlay.classList.add('hidden');return;}
    GameState.continuousPlay.cityScores.push({city:GameState.currentCity?.name||'Unknown',score:GameState.totalScore});
    GameState.continuousPlay.citiesCompleted++;
    ...
    GameState.map.stop();
    GameState.map.jumpTo({ center: [nextCity.lng, nextCity.lat], zoom: nextCity.zoom || 15,pitch:0,bearing:0 });
    ...
    // Use atomically captured preloaded data if available
    if (usedPreloadedData) {
        processRoadData(usedPreloadedData);
    ...
    if(GameController.phase===GamePhase.PLAYING){CityFacts.startTicker(nextCity);scheduleNextRoundEndpointPrecompute();preloadNextCity();}
}
...
function disableContinuousPlay() {
    GameState.cityTransitionId=(GameState.cityTransitionId||0)+1;
    document.getElementById('city-transition')?.classList.add('hidden');
...
function getLobbyCityPreparation(){
    if(!lobbyCityPreparation)lobbyCityPreparation=PathfindrLobbyPreload.create({
        ...
        save:(mode,reserve)=>RoadNetworkCache.set(`lobby-reserve-v3:${mode}`,{city:reserve.city,elements:reserve.data.elements,scene:reserve.scene}),
...
function showResults() {
    ...
    // Reuse the city facts warmed during play; do not delay the results for a fetch.
    void CityFacts.showFactInResults(GameState.currentCity).catch(()=>{});
...
function drawVirtualEdgesOverlay(ctx, width, height, intensity = 1) {
    intensity *= getAmbientRoadVisibility();
    if (!intensity) return;
 ...
 */
function getAmbientRoadVisibility() {
    if (window.PathfindrCity?.state.enabled === false) return 1;
    const t = Math.max(0, Math.min(1, ((GameState.map?.getPitch?.() || 0) - 3) / 22));
    return 1 - t * t * (3 - 2 * t);
}

function drawRoadNetwork(ctx) {
    if (!getAmbientRoadVisibility()) return;
...
function exitToMenu() {
    ...
    // Keep phase-owned HUD/overlay visibility in sync with the lobby. Merely
    // showing its DOM lets the next gameplay frame cover it again.
    GameController.enterPhase(GamePhase.MENU);
    showModeSelector();
}
