...
(() => {
    ...
    async function publish(payload,requestId){if(!auth()?.currentUser)throw Error('Sign in for hosted sharing. Portable codes and files work without an account.');const {data,error}=await client().rpc('publish_route_share',{p_request_id:requestId,p_payload:PathfindrShareData.validate(payload)});check(error);return data;}
