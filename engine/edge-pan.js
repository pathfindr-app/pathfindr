...
(() => {
    function zones(rect,padding={}){
        ...
        return {x:Math.min(260,Math.max(96,width*.36),width*.4),y:Math.min(280,Math.max(120,height*.34),height*.4)};
    }
    function velocity(point,rect,padding={},band){
        ...
        const axis=(p,a,b,zone)=>{zone=Math.min(zone,(b-a)*.4);if(zone<=0)return 0;
