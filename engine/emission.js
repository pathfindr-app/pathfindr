...
(() => {
    let gl, width=0,height=0,targets=[],programs,quad,type;
            ...
            try{
                ...
                if(!programs){
                    ...
                    programs={blur:program(`precision highp float; varying vec2 uv; uniform sampler2D source; uniform vec2 direction;
                        ...
                        composite:program(`precision highp float;varying vec2 uv;uniform sampler2D core;uniform sampler2D nearGlow;uniform sampler2D midGlow;uniform sampler2D farGlow;
                        ...
                        vec3 mapped=vec3(1.0)-exp(-(c+glow)*.8);gl_FragColor=vec4(mapped,clamp(max(mapped.r,max(mapped.g,mapped.b)),0.0,1.0));}`)};
