    ...
    public class MoveMarkerOnRouteExample : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            OnlineMapsGoogleRouting request = new OnlineMapsGoogleRouting(
                new OnlineMapsGoogleRouting.Params(fromPlace, toPlace)
                {
                    key = googleAPIKey
                }
            );
        ...
        private void OnComplete(string response)
        {
            ...
            OnlineMapsGoogleRoutingResult.Route firstRoute = result.routes[0];
            List<OnlineMapsGoogleRoutingResult.RouteLegStep> steps = firstRoute.legs.SelectMany(l => l.steps).ToList();

            // Create a new marker in first point.
            marker = map.markerManager.Create(steps[0].startLocation, "Car");
