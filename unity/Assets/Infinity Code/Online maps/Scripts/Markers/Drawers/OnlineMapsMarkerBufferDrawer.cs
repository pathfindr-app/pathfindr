...
public class OnlineMapsMarkerBufferDrawer : OnlineMapsMarker2DDrawer
{
    ...
    private void Draw()
    {
        if (!control.markerManager) return;
        ...
        OnlineMapsMarker[] markers;
        lock (control.markerManager)
        {
            ...
            markers = usedMarkers.ToArray();
        }       
        foreach (OnlineMapsMarker marker in markers) SetMarkerToBuffer(marker, bufferPosition, bufferZoom, frontBufferPosition, sx, sy, ex, ey);
    }
    ...
    internal class MarkerComparer : IComparer<OnlineMapsMarkerBase>
    {
        public int Compare(OnlineMapsMarkerBase m1, OnlineMapsMarkerBase m2)
        {
            ...
            double oy = m1y - m2y;
            return Math.Abs(oy) < double.Epsilon ? Math.Sign(m2x - m1x) : Math.Sign(oy);
        }
    }
}