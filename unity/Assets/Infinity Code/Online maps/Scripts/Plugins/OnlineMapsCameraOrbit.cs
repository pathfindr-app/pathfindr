...
public class OnlineMapsCameraOrbit : MonoBehaviour, IOnlineMapsSavableComponent
{
    ...
    private void Update()
    {
        ...
        if (OnlineMapsInput.GetMouseButton(1))
        { 
            Vector2 inputPosition = control.GetInputPosition();
#else
        if (OnlineMapsInput.touchCount == 2)
        {
            Vector2 p1 = OnlineMapsInput.GetTouch(0).position;
            Vector2 p2 = OnlineMapsInput.GetTouch(1).position;
