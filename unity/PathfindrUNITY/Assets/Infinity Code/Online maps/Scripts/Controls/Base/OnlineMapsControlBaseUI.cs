...
public abstract class OnlineMapsControlBaseUI<T> : OnlineMapsControlBase2D where T: MaskableGraphic
{
    ...
    protected override void BeforeUpdate()
    {
        ...
        int touchCount = OnlineMapsInput.GetMouseButton(0) ? 1 : 0;
...
#else
        if (OnlineMapsInput.touchCount != lastTouchCount)
        {
            if (OnlineMapsInput.touchCount == 1) OnMapBasePress();
            else if (OnlineMapsInput.touchCount == 0) OnMapBaseRelease();
        }
        lastTouchCount = OnlineMapsInput.touchCount;
#endif
    }
