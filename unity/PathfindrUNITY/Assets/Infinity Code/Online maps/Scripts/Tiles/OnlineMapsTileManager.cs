...
public class OnlineMapsTileManager
{
    ...
    public void StartDownloading()
    {
        ...
        int maxDownloads = maxTileDownloads;
        if (map.activeType.provider.maxDownloads > 0)
        {
            maxDownloads = Mathf.Min(map.activeType.provider.maxDownloads, maxDownloads);
        }
        ...
        int needDownload = maxDownloads - countDownload;

        if (downloadTiles == null || downloadTiles.Length < maxDownloads)
        {
            downloadTiles = new OnlineMapsTile[maxDownloads];
        }
        ...
        for (int i = 0; i < c; i++)
        {
            ...
            countDownload++;
            if (countDownload > maxDownloads) break;
