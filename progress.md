...
## September 7 — forgiving Circuit keyboard steering
- Newest held key wins; OS repeat cannot override a turn. Same-road / geometric continuity follows gentle bends. Early turn assistance is bounded to 600ms and 65 screen pixels; release/pause stops immediately. Lookahead stops at real junctions, with a weak local endpoint tie-break rather than automatic route solving. Junction cadence eases; reversing erases and a keyboard stroke is one Undo.
- Finished player routes are 4.2px optical base versus active 9px, retaining animated dash-dot identity. Real A* remains player-color locked. Bot optimal suffixes reuse cached baseline analysis when unobstructed; barriers/repair retain actual searches.
- Research and tuning rationale: docs/circuit-input-ux.md. Full suite 183/183 pass; diff whitespace clean. In-app latest build started and rendered correctly, then paused. Prior keyboard/trace/finish UI checks and screenshots remain applicable to their covered behavior. Latest stock headless run was excessively slow and did not complete; it is not a passing end-to-end keyboard test or physical-device performance evidence.
- Preview dpl_J74MEUF14R9wZb52fT3Uoy3HMHc8 / pathfindralpha-nydt5vwmj-pathfindr-apps-projects.vercel.app: seven Arena files byte-verified, authenticated main/fallback/storefront/builder/private-source checks pass. Stage /tmp/pathfindr-arena-controls-tIJsKK preserves all 308 non-Arena source files from verified current .47 production. Released as dpl_C6ZnpdoVs7d4bu4nx6XbAR15suZh; public seven-file, Classic entry/immersive/manifest and all eight Printshop checks pass. See RELEASE-ARENA-CONTROLS.md for rollback and verification limits.

## September 7 — Circuit controls/spec correction (.48 arena only)
- User rejected the vehicle-like movement queue and shared ordered destinations. Clarified five fixed S/E pairs, each player initially on a different pair; also requested WASD/arrows and Classic-like visualizations with prominent color-matched player/S/E markers.
- Protected prior source at codex/arena-before-direct-controls-20260907 (460c168). Main Classic/Visualizer and Printshop are untouched. Circuit now synchronously edits paths, reuses Classic trace lifecycle/backtracking/corridor/edge-pan helpers, supports directional keyboard tracing, and undoes erased strokes. Subdivides real edges for precision without inventing crossing connections.
- Five fixed route pairs, starts >=1.2km apart, distinct initial ends. Each player completes all five; allocator never assigns an occupied pair. Explicit limitation: if remaining pairs are occupied, a player waits; idle-human cases use the eight-minute timeout. No claim of online play or ranked fairness.
- Actual A* baseline results/expansion trees cached per pair; finish holds both endpoints for a 3.2s replay, shows measured distance/score, then assigns another pair without a joining line. Same player color for head shape, S/E, path and search; shared Classic route-cinema/afterglow/fidelity. Sampled search geometry and cached projections bound rendering work; removed repeated HUD snapshot serialization. No physical iPad performance claim; base terrain is still Canvas, not full Classic WebGL renderer.
- 176 tests pass including real Cairo match completion, exclusive pair assignment, immediate/no-queue edits, WASD/arrows + reverse erase, undo after erasing, pause/replay, ability validation, and actual A* expanded edges/score/reveal hold. Stock client produced output/arena-final-controls gameplay screenshot/state without errors, inspected. Actual browser tap completion: 1.31km vs 1.31km,1000pts, visible cyan A* branches and pause/resume. Distinct rival shapes/colors visible later. Viewport capability produced distorted captures; isolated 390x844 iframe verifies actual mobile responsive layout instead. No claim that emulation is physical-device testing.
- Preview dpl_3PGFccT4gHmVTdkhm49XSE6afUVH at pathfindralpha-5xshe5wlq-pathfindr-apps-projects.vercel.app; exact stage /tmp/pathfindr-arena-controls-WEvf6B from .47 dpl_8gTog7HcR5jtgqfeizAXHhfhXBMc. All 308 non-Circuit source files preserved; eight shared engine dependencies byte-matched to tested source. Preview six Circuit files exact-match, authenticated Classic/fallback/Printshop/private-source checks pass, public baseline Printshop checks pass. Production promotion pending.

## September 7 — .47 completed brief
- Live dpl_8gTog7HcR5jtgqfeizAXHhfhXBMc, build pathfindr-bounded-scenery-20260907.47, stage /tmp/pathfindr-bounded-scenery-EMadYw. Source 7fb4457. RELEASE-BOUNDED-SCENERY.md records source-byte checks, preview/public Printshop checks, exact rollback and limitations.
- Root cause found: Boston's Gulf of Maine relation had 421,074 coordinates across hundreds of km, sent whole to triangulation. D3 rectangle clipping preserves shoreline/island topology; preclipped 36 packs reduce Boston to 14,485 surface points / 3.36MB. Road hashes unchanged; scene cache revision added. Actual Boston mobile gameplay and desktop water rendering verified. Normal US startup then opens Portland; Explorer Global opens New Cairo; Visualizer Washington Pause/Play/exit verified.
- Challenge failure guard rendered correctly; fixed its bogus Resume Run by normal exit cleanup. Test fixture hashes source to avoid stale local script cache. 169 tests pass. Stock headless navigation remains an unsuccessful test, not runtime evidence.
- GOAL-OVERNIGHT.md acceptance audit completed using current source plus preserved earlier release/runtime evidence. Read-only DB audit and 100-key catalog remain delivered artifacts; diagnostics migration/achievement ledger/online authority are explicitly future work, not silently done. No Printshop/payment/database changes in .47.
...
- UI follow-up: actual Cut→KITE succeeded (5→1 charge, explicit repair message); safe Barrier accepted (3→0, visible 100m radius/10-second countdown); accelerated normal simulation reached final results with all three bots finishing; Race again reset time/goals/charge. Added finish times to final standings to explain first-finisher ranking. Final real-New-Cairo human-command test completes all five and wins; 152 tests pass before last presentation-only refinements. Prototype not deployed yet.
- Audit/catalog update: docs/achievement-catalog.md contains exactly 100 unique-key proposals with event contracts/thresholds (count checked). docs/persistence-audit-20260907.md records actual read-only live SQL findings: games1063/routes1060, replays0, route_runs4/15rounds/13collectionitems (all canonical keys, no brands), shares1, challenge_entries78/no v2 payloads. route_issues table and RPC absent. RLS policies inspected; source files byte-matched against prod. No database writes/migrations. These are concrete gaps, not “everything saved” certification.
- Separate worktree Pathfindr-arena-20260907, branch codex/new-cairo-arena-20260907 from protected 59e1a12 (.45). arena/core.js is deterministic fixed-tick authority with OSM graph, five shared goals, four distinct ~800m starts, three modest bots, command sequences, charge economy, safe timed barriers, cut/repair/protection, meter-bounded shortcuts, ranking/end state, snapshot restore and local transport. arena/app.js + index/CSS are isolated canvas/client/controls; Classic and Printshop untouched.
- Actual in-app browser: loaded New Cairo, collected charge, planned first route and reached destination 1 with 1,000 score/+2 charge, paused/resumed. Mobile 390x844 briefing/control inspection, collected first charge, successfully targeted shortcut (5→0 charge, “Shortcut engaged”). Reset viewport override afterward.
- Stock web-game client runs captured gameplay and state without error files; both screenshots opened and inspected. Base roads toned down; animated route dashes clearer; reached/repair event automatically reframes head and target. Unit suite 151 passing before final real-map human-completion test addition; rerun after that test. Local server4202 session58309.
- Prototype still needs broader browser checks of barrier/cut/repair, full end/restart flow, real-device performance, deployment/entry access, and final scope audit. README describes rules and authoritative-network extension accurately (not online multiplayer yet). Live DB audit and 100-achievement catalog remain outstanding. Dashboard authenticated successfully in existing browser auditTab at Supabase wxlglepsypmpnupxexoc; no SQL run yet, no schema changes. Do not mark overall goal complete.

### Active Visualizer continuity goal — September 7, unreleased
- RELEASE UPDATE: main .45 is live, dpl_Cx4CYnfmrzSPEpHH2WQgZfQyHwYW, stage /tmp/pathfindr-continuity-gEweRG, source bce2434 pushed on codex/visualizer-continuity-20260907. See RELEASE-CONTINUITY.md. Production Printshop checks all pass. 36 packs finished (New Cairo succeeded with build-only 120s timeout), catalog regenerated; 142 tests pass. Boston→Paris disconnected-provider handoff verified with actual new graph/scenery/labels at pitch zero. Below chronological unfinished notes are superseded by this release update, not by a claim that the entire goal is complete.
...
- Reuse route camera projection arrays, cull ineligible label projections, cap coarse-pointer MapLibre raster at 1.5 DPR / 2.4M pixels (route/UI buffers unchanged). No measured iPad FPS claim. Existing look retained.
- Polygon-boundary barycentric AA softens water/park perimeter, leaves internal triangulation seams opaque. Added eight slow coordinate-number travelers clipped outside expanded road bounds; geometric clip avoids full-screen alpha-mask target. Reduced-motion hides travelers.
- Game AdSense script removed, ad initialization/preloads/presentation disabled by config, rewarded ads off. Removed two lobby arrow decorations. Printshop untouched.
- 130 game tests pass. Stock client captured actual bundled Miami Visualizer with 4000 buildings, 62 surfaces, 247 labels, no shader/render-loop errors; screenshots inspected. In-app screenshot verified terrain/labels/search and Escape exit. Real random-city stock test captured loading (not successful gameplay); live Overpass probe returned HTTP429. Cold first visits remain network dependent; local Supabase CORS errors also persist. Do not claim rate limits or all tablet jank eliminated.
- Performance references: https://maplibre.org/maplibre-gl-js/docs/API/classes/Map/ and https://threejs.org/manual/en/optimize-lots-of-objects.html .
- Final release stage /tmp/pathfindr-lobby-perf-vh9Um2 cloned and SHA1-verified current production dpl_4nrDGPZqRHSX1FDCqvvwSGfuSJUR, preserved 256 unrelated files including Printshop/functions. Final preview dpl_CckiEL2XY1aESWzG3fxxqeoXBQ87 passed authenticated storefront/builder/assets/policies/private-source/game-entry checks, plus exact config/ads byte verification. Standard preview checker encounters Vercel login. Production baseline rechecked unchanged before promotion.
- Follow-up guards: returning to lobby invalidates pending road loads; ad suspension is kept separate from paid entitlements (isAdFree is also used for premium city search). 130 tests still pass after both safeguards.
- Live .44: dpl_AAk8jKraGafbD89GKDmjz1hcox8Y, https://www.pathfindr.world. All 11 changed public game/version files byte-verified against final stage after deployment. Prior exact shared deployment remains dpl_4nrDGPZqRHSX1FDCqvvwSGfuSJUR; do not use older Printshop baselines for rollback.
...
- Live .43 deployment dpl_4nrDGPZqRHSX1FDCqvvwSGfuSJUR at www.pathfindr.world. Preview authenticated checks passed (ordinary checker hit preview sign-in); production Printshop checker passed all eight checks, and all four changed public game files were byte-verified against the staged release.
