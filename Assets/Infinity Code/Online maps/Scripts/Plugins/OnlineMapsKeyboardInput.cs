...
public class OnlineMapsKeyboardInput : MonoBehaviour, IOnlineMapsSavableComponent
{
    ...
    private void Start()
    {
        ...
        map.OnChangePosition += UpdateTilePosition;
        map.OnChangeZoom += UpdateTilePosition;
    }

    private void Update()
    {
        float latitudeSpeed = OnlineMapsInput.GetAxis("Vertical") * Time.deltaTime;
        float longitudeSpeed = OnlineMapsInput.GetAxis("Horizontal") * Time.deltaTime;
