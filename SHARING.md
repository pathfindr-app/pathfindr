...
- Share on round/run results opens one direct link to the selected round or full run, including recorded routes and results with Copy link and native Share when supported. Compact shares are self-contained; local/native previews use https://www.pathfindr.world. Larger shares automatically request a hosted link for signed-in players; this still requires migration 016. The main share flow no longer exposes publishing, code, or download choices. Existing link/code/file imports remain available.
...
`route_runs` is owner-only via RLS. `route_shares` is immutable and unlisted: guest access is a lookup-only function, not table enumeration. Clicking Share authorizes link creation; large payloads use hosted publishing automatically when signed in. Owner revocation RPC is present; a share-management UI is not yet implemented. Local data removal does not revoke a published share.
